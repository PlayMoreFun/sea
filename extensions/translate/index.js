...
formatMessage.setup({
    translations: {
        en: {
            ...
            translateBlock: 'translate [WORDS] to [LANGUAGE]',
            defaultTextToTranslate: 'hello',
            viewerLanguage: 'language'
        },
        'zh-cn': {
            categoryName: '翻译',
            translateBlock: '将 [WORDS] 译为 [LANGUAGE]',
            defaultTextToTranslate: '你好',
            viewerLanguage: '访客语言'
        },
        'zh-tw': {
            categoryName: '翻譯',
            translateBlock: '文字 [WORDS] 翻譯成 [LANGUAGE]',
            defaultTextToTranslate: 'hello',
            viewerLanguage: '瀏覽者的語言'
        }
    }
})
...
// 通过百度申请
const appid = '20220420001181526';
const secret = 'iHyoFL0_jWOcOIbJmJe3';
...
class TranslateBlocks {
    ...
    getViewerLanguage () {
        ...
        // 部分百度支持的语言代码与 Scratch 的不同，需要进行转换
    ...
    getTranslate (args) {
        ...
        const signStr = `${appid}${args.WORDS}${salt}${secret}`;

        let urlBase = `${serverURL}?appid=${appid}&from=auto`;
        urlBase += `&to=${lang}`;
        ...
        const self = this;
        const translatePromise = fetchJsonp(urlBase)
            .then(res => res.json())
